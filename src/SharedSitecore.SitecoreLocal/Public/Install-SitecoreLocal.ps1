...
function Install-SitecoreLocal {
    ...
    Param (
		...
		[Parameter(Mandatory=$false,Position=0)] [string]$name = 'dev',
	...
	begin {
		...
		Write-Host "PSRepoPath:$PSRepoPath"
	...
	`	$logPath = Join-Path $LogFolder "$hostname-$dateFormat.log"
		...
		if(!$packages){ $packages = Join-Path $PSRootDrive 'repos\docker-images\build\packages' }
	...
	process {
		try {
			if($PSCmdlet.ShouldProcess($ConfigurationFile)) {
				...
				if(!$scPkg){$scPkg = (Get-ChildItem "$packages\Sitecore 9* rev. * (OnPrem)_single.scwdp.zip").FullName }
				if(!$idPkg){$idPkg = (Get-ChildItem "$packages\Sitecore.IdentityServer * rev. * (OnPrem)_identityserver.scwdp.zip").FullName }
				if(!$xcPkg){$xcPkg = (Get-ChildItem "$packages\Sitecore 9* rev. * (OnPrem)_xp0xconnect.scwdp.zip").FullName	}
			
			
				#if (!(Test-Path $ConfigurationRoot)) {
			#		mkdir $ConfigurationRoot #| Out-Null
			#	}
				...
				$testConfigFile = Join-Path $ConfigurationRoot "XP0-SingleDeveloper.json"
				#missing config #todo: add or for -clean?
				...
				#read config first? installers need info? or last incase they make changes
